...
def painel_executivo():
    ...
    anos=sorted({int(y) for y in vendas_raw["dt"].dropna().dt.year.tolist()}|{ano_atual},reverse=True); c1,c2,c3=ui.st.columns([1.48,.9,.98],gap="small")
    with c1:
        with ui.st.container(border=True):
            ...
            mensal=pd.DataFrame({"mes":range(1,13),"Mês":nomes}).merge(mensal_sum,on="mes",how="left").fillna({"Vendas":0.0}); mensal["Valor"]=mensal["Vendas"].apply(_moeda_br_num); x_axis=alt.Axis(labelAngle=-50,labelOverlap=False,labelLimit=95,values=nomes,title=None)
            bars=alt.Chart(mensal).mark_bar(color="#54b8f4",cornerRadiusTopLeft=4,cornerRadiusTopRight=4).encode(x=alt.X("Mês:N",sort=nomes,axis=x_axis),y=alt.Y("Vendas:Q",title="Vendas (R$)"),tooltip=[alt.Tooltip("Mês:N",title="Mês"),alt.Tooltip("Valor:N",title="Vendas")]); labels=alt.Chart(mensal).mark_text(dy=-8,color="#f2f7fb",fontSize=9).encode(x=alt.X("Mês:N",sort=nomes,axis=x_axis),y="Vendas:Q",text=alt.Text("Valor:N")).transform_filter("datum.Vendas > 0"); ui.st.altair_chart(_chart_theme((bars+labels).properties(height=305)),use_container_width=True); ui.st.caption("Valores em R$ (Real) • Janeiro a Dezembro")
