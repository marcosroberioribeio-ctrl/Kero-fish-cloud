...
def painel_executivo():
    ...
    with c1:
        with ui.st.container(border=True):
            ...
            bars=alt.Chart(mensal).mark_bar(color="#54b8f4",cornerRadiusTopLeft=3,cornerRadiusTopRight=3,size=24).encode(x=alt.X("Mês:N",sort=nomes,axis=x_axis),y=alt.Y("Vendas:Q",title="Vendas (R$)"),tooltip=[alt.Tooltip("Mês:N",title="Mês"),alt.Tooltip("Valor:N",title="Vendas")]); labels=alt.Chart(mensal).mark_text(dy=-7,color="#f2f7fb",fontSize=9).encode(x=alt.X("Mês:N",sort=nomes,axis=x_axis),y="Vendas:Q",text=alt.Text("Valor:N")).transform_filter("datum.Vendas > 0"); ui.st.altair_chart(_chart_theme((bars+labels).properties(height=285)),use_container_width=True); ui.st.caption("Valores em R$ (Real) • Janeiro a Dezembro")
