...
def _cep_fields(ui, prefix: str) -> dict[str, str]:
    ...
    if c2.button("🔎 Buscar CEP", key=f"{prefix}_buscar_cep", use_container_width=True):
        try:
            ...
            st.session_state[f"{prefix}_cep"] = dados["cep"]
            st.session_state[f"{prefix}_endereco"] = dados["logradouro"]
            st.session_state[f"{prefix}_bairro"] = dados["bairro"]
            st.session_state[f"{prefix}_cidade"] = dados["cidade"]
            st.session_state[f"{prefix}_uf"] = dados["uf"]
            if dados["complemento"] and not st.session_state.get(f"{prefix}_complemento"):
                st.session_state[f"{prefix}_complemento"] = dados["complemento"]
    ...
    endereco = c3.text_input("Rua / Logradouro", key=f"{prefix}_endereco")
    numero = c4.text_input("Número", key=f"{prefix}_numero")
    c5, c6, c7 = st.columns([2, 2, 1])
    bairro = c5.text_input("Bairro", key=f"{prefix}_bairro")
    cidade = c6.text_input("Cidade", key=f"{prefix}_cidade")
    uf = c7.text_input("UF", key=f"{prefix}_uf", max_chars=2)
    complemento = st.text_input("Complemento", key=f"{prefix}_complemento")
    return {
        "cep": formatar_cep(st.session_state.get(f"{prefix}_cep", cep)),
